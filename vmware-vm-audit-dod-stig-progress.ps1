...
# --- Collect VMs ----------------------------------------------------------
Write-Output "📊 Collecting VM inventory..."
$startTime = Get-Date
...
foreach ($vm in $vms) {
  $currentVM++
  $percentComplete = [math]::Round(($currentVM / $totalVMs) * 100, 1)
  $elapsedTime = (Get-Date) - $startTime
  $avgTimePerVM = if ($currentVM -gt 1) { $elapsedTime.TotalSeconds / ($currentVM - 1) } else { 0 }
  $estimatedTimeRemaining = if ($avgTimePerVM -gt 0) { [TimeSpan]::FromSeconds($avgTimePerVM * ($totalVMs - $currentVM)) } else { [TimeSpan]::Zero }
  
  Write-Progress -Activity "🔍 Auditing VMs for DoD STIG compliance" -Status "Processing: $($vm.Name) ($currentVM of $totalVMs)" -PercentComplete $percentComplete -CurrentOperation "$percentComplete% complete | ETA: $($estimatedTimeRemaining.ToString('mm\:ss'))"
  Write-Output "[$currentVM/$totalVMs] ($percentComplete%) Auditing: $($vm.Name)"
...
Write-Progress -Activity "🔍 Auditing VMs for DoD STIG compliance" -Completed
$totalTime = (Get-Date) - $startTime
Write-Output "✅ Audit completed in $($totalTime.ToString('mm\:ss')) (mm:ss)"
...
<head>
    ...
    <style>
        ...
        th:nth-child(8), td:nth-child(8) { min-width: 300px; } /* Non-Compliant Reasons */
...
<body>
    ...
    <div class="summary">
        ...
        <p><strong>Audit Duration:</strong> $($totalTime.ToString('mm\:ss'))</p>
    </div>
    <table>
        <tr>
            <th>VM Name</th>
...
Write-Output "   ⚠️  Non-Compliant: $nonCompliantCount"
